...
- **`wt0 doctor`'s human-readable output is a redesigned before/after
  ...
  additive keys. The cost table gains a fourth "saving" column and the
  Tilt-only line becomes a "🎛️ Dev environment" block naming every
  dev-environment tool detected, not just Tilt.
